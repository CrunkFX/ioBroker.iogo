...
class MessageSendService{
    ...
    send(obj){
        ...
        if (obj.message) {
            ...
            if (typeof obj.message === 'object') {
                count = this.sendMessage(obj.message.text, obj.message.user, obj.message.title, obj.message.url, obj.message.duration);
            } else {
    ...
    sendMessage(text, username, title, url, duration) {
        if (!text && text !== 0) {
        ...
        let re = new RegExp("^([0-9]+[smhd]{1})$");
        if(duration === undefined || duration !== typeof 'string' || !re.test(duration)){
            this.adapter.log.warn('duration format ' + duration + ' is invalid');
            duration = null;
        }
        ...
        if (text && (typeof text === 'string' && text.match(/\.(jpg|png|jpeg|bmp)$/i) && (fs.existsSync(text) ))) {
            this._sendMessageHelpersendImage(text, messageKey).then((downloadurl) => {
                this.sendMessageToUser(null, username, title, messageKey, downloadurl, text, duration)
            });
            ...
            this.sendImage(url, messageKey).then((downloadurl) => {
                this.sendMessageToUser(text, username, title, messageKey, downloadurl, url, duration)
            });
        }else{
            this.sendMessageToUser(text, username, title, messageKey, null, null, duration)
        }
    }
    ...
    sendMessageToUser(text, username, title, messageKey, url, filename, duration){
        let count = 0;
        let u;
        let recipients = this.getFilteredUsers(username);
    
        for (u in recipients) {
            count += this._sendMessageHelper(this.users[u], u, text, title, messageKey, url, filename, duration);
        }
        return count;
    }
    
    _sendMessageHelper(token, username, text, title, messageKey, url, filename, duration) {
        if (!token) {
